...
def test_deallocation(sqlite_session_factory):
    uow = unit_of_work.SqlAlchemyUnitOfWork(sqlite_session_factory)
    sqlite_bus.handle(commands.CreateBatch("b1", "sku1", 50, None), uow)
    sqlite_bus.handle(commands.CreateBatch("b2", "sku1", 50, today), uow)
    sqlite_bus.handle(commands.Allocate("o1", "sku1", 40), uow)
    sqlite_bus.handle(commands.ChangeBatchQuantity("b1", 10), uow)
