...
class FakeMessageBus(messagebus.AdstractMessageBus):
    def __init__(self):
...
class TestAllocate:
    def test_returns_allocation(self):
        ...
        results = messagebus.messagebus.handle(
...
class TestChangeBatchQuantity:
    def test_changes_available_quantity(self):
        ...
        messagebus.messagebus.handle(events.BatchQuantityChanged("batch1", 50), uow)
    ...
    def test_reallocates_if_necessary(self):
        ...
        for e in event_history:
            messagebus.messagebus.handle(e, uow)
        [batch1, batch2] = uow.products.get(sku="INDIFFERENT-TABLE").batches
        assert batch1.available_quantity == 10
        assert batch2.available_quantity == 50

        messagebus.messagebus.handle(events.BatchQuantityChanged("batch1", 25), uow)
...
def test_reallocates_if_necessary_isolated():
    ...
    reallocation_event = mb.events_published[-1]
