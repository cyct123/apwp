...
class TestAllocate:
    def test_allocates(self):
        ...
        bootstrap_test_app.handle(commands.Allocate("o1", "COMPLICATED-LAMP", 10), uow)
    ...
    def test_commits(self):
        ...
        bootstrap_test_app.handle(commands.Allocate("o1", "OMINOUS-MIRROR", 10), uow)
        assert uow.committed
...
class TestChangeBatchQuantity:
    def test_changes_available_quantity(self):
        ...
        bootstrap_test_app.handle(commands.ChangeBatchQuantity("batch1", 50), uow)
    ...
    def test_reallocates_if_necessary(self):
        ...
        for msg in history:
            bootstrap_test_app.handle(msg, uow)
        [batch1, batch2] = uow.products.get(sku="INDIFFERENT-TABLE").batches
        assert batch1.available_quantity == 10
        assert batch2.available_quantity == 50

        bootstrap_test_app.handle(commands.ChangeBatchQuantity("batch1", 25), uow)
